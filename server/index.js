...
// Connect DB & Seed if empty
connectDB().then(async () => {
  const prodCount = await Product.countDocuments();
  if (prodCount === 0) {
    console.log('No products found in MongoDB. Initializing seed data...');
    await seedDatabase();
  }
});
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    // Security: Strip expectedPrice before returning to vendor client
...
// 3. Get Active Contractors (Vendors)
...
// 4. Save Request (Server-side calculation & MongoDB Save)
app.post('/api/requests', async (req, res) => {
  try {
    ...
    // Look up property details if available
    ...
    // Build product ID list to query database directly for accurate server-side pricing
...
// Admin Get Products (with expected prices)
...
app.listen(PORT, () => {
  console.log(`🚀 Property Materials Server running on port ${PORT}`);
});
